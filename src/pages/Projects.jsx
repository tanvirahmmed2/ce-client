...
const Projects = () => {
    ...
    return (
      <section className="w-full min-h-[800px] bg-slate-50 p-6 flex flex-col items-center justify-center gap-8">
       ...
       <div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 justify-items-center gap-8'>
        {
          projects && projects.map((project)=>{
            ...
            return <motion.div initial={{opacity:0}} whileInView={{opacity:1}} transition={{duration: 0.6}} key={_id} className='w-full flex flex-col items-start justify-between gap-2  shadow-xl rounded-lg p-4 border-2 hover:scale-[1.02] transition duration-500'>
              ...
              <p>{description.slice(0,20)} .....<Link to={`/projects/${_id}`} className='text-red-500'>show more</Link> </p>
