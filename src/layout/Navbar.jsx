  ...
  return (
    <nav className='w-full h-16 fixed flex flex-row items-center justify-between px-2 md:px-6 shadow-lg z-40 bg-white text-emerald-950 border-opacity-50'>
      ...
      <div className='w-auto h-full flex flex-row items-center justify-center gap-2'>
        {
          user? <Link to='/profile'><img src={user.profileImage} alt=""  className='w-10 h-10 rounded-full'/></Link>: <Link Link to='/login' className='w-auto p-2 hidden md:flex justify-center items-center px-4 bg-emerald-600 text-white transition duration-500 hover:bg-emerald-400 rounded-lg'>Login</Link>
        }
