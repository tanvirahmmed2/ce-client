...
const ProjectHandle = () => {
  ...
  const [editData, setEditData] = useState({ editId: null, title: '', description: '' })
  ...
  const editProject = async (e) => {
    e.preventDefault()
    try {
      const response = await axios.put(`${api}/project/update`, editData, { withCredentials: true })
      toast.success(response.data.message)
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-6">
          ...
          <div className="flex flex-col gap-2">
            <label htmlFor="image" className="font-medium">
              Image <span className="italic text-xs font-normal text-red-400">(can't change later)</span>
            </label>
            <input
              type="file"
              name="image"
          ...
          <button type="submit" className="w-full bg-emerald-600 hover:bg-emerald-700 transition text-white font-semibold py-2 rounded-lg shadow-md">
            Publish
          </button>
        </form>
      </div>
        ...
        {projects && projects.length > 0 ? (
          <div className="flex flex-col gap-4">
            {projects.map(({ _id, title, description }) => (
              <div key={_id} className="border rounded-xl shadow-sm hover:shadow-md transition-all p-4 bg-white flex flex-col gap-3">
                <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-2">
                  <Link to={`/projects/${_id}`} className="text-lg font-semibold text-emerald-700 hover:underline">
                    {title}
                  </Link>
                  <div className="flex gap-4">
                    <button onClick={() => removeProject(_id)} className="text-red-500 hover:text-red-700 hover:scale-110 transition">
                ...
                {editData.editId === _id && (
                  <div  className="mt-3 border-t pt-3 flex flex-col gap-3">
                    <h2 className="font-semibold text-gray-700">Edit Project</h2>
                    <input
                      type="text"
                      name="title"
                    ...
                    <div className="flex gap-3">
                      <button onClick={editProject} className="bg-blue-600 text-white px-4 py-1 rounded-lg hover:bg-blue-700 transition">
                        Save
                      </button>
                      <button
                        type="button"
                        onClick={() => setEditData({ editId: null, title: "", description: "" })}
          ...
          <p className="p-4 text-center text-gray-500">No project uploaded yet.</p>
        )}
      </div>
    </div>
  )
